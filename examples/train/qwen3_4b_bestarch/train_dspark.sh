#!/bin/bash
# DSpark: AdamW, paper LR/decay/schedule, and full attention.
...
OUTPUT_DIR="./output/qwen3_4b_bestarch/dspark_custom"
RUN_NAME="dspark_custom"
VLLM_PORT=8001
...
nohup env ASCEND_RT_VISIBLE_DEVICES="$TRAIN_NPUS" torchrun \
    ...
    --scheduler-type "$SCHEDULER_TYPE" --scheduler-warmup-ratio "$WARMUP_RATIO" \
    ...
    --total-seq-len "$SEQ_LENGTH" --speculator-type dspark \
