"""Lightweight Muse option registration and training-parser boundary contracts."""
...
from speculators.models.muse.config import MUSE_OPTION_FIELDS, muse_option_defaults
from speculators.train import cli, muse_args
from speculators.utils.argparse_utils import explicitly_provided_dests


def _parser(defaults=None):
    parser = argparse.ArgumentParser(add_help=False)
    values = muse_option_defaults() if defaults is None else defaults
    muse_args.add_muse_backbone_args(parser, values)
    muse_args.add_muse_correction_args(parser, values)
    return parser
...
def test_registration_groups_cover_only_schema_owned_destinations():
    groups = []
    for register in (
        muse_args.add_muse_backbone_args,
        muse_args.add_muse_correction_args,
    ):
        parser = argparse.ArgumentParser(add_help=False)
        assert register(parser, muse_option_defaults()) is None
        groups.append({action.dest for action in parser._actions})
    assert groups[0].isdisjoint(groups[1])
    assert groups[0] | groups[1] == MUSE_OPTION_FIELDS
    excluded = {
        "markov_rank",
        "markov_head_type",
    ...
    assert MUSE_OPTION_FIELDS.isdisjoint(excluded)
    assert vars(_parser().parse_args([])) == muse_option_defaults()


def test_registration_consumes_custom_read_only_defaults_without_mutation():
    original = muse_option_defaults()
    alternate_strings = {
    ...
    assert muse_option_defaults() == original


def test_each_boolean_option_accepts_both_spellings_and_tracks_explicit_defaults():
    parser = _parser()
    assert explicitly_provided_dests(parser, MUSE_OPTION_FIELDS, []) == set()
    actions = [
        action
    ...
    for action in actions:
        ...
        for flag in (positive, negative):
            assert explicitly_provided_dests(parser, MUSE_OPTION_FIELDS, [flag]) == {
                action.dest
            }
...
def test_search_switches_are_mutually_exclusive_with_caller_owned_default(default_mode):
    defaults = {**muse_option_defaults(), "dflash2_selector_search_mode": default_mode}
...
parser = argparse.ArgumentParser(add_help=False)
module['add_muse_backbone_args'](parser, defaults)
module['add_muse_correction_args'](parser, defaults)
...
"""
    result = subprocess.run(  # noqa: S603 -- Trusted Python and repository module; no shell.
        [
            ...
            program,
            muse_args.__file__,
            json.dumps(muse_option_defaults()),
        ],
        capture_output=True,
        text=True,
        timeout=30,
...
def test_full_parser_shares_one_defaults_mapping_between_registration_groups(
    ...
    defaults = muse_option_defaults()
    supplier = Mock(return_value=defaults)
    backbone = Mock(wraps=muse_args.add_muse_backbone_args)
    correction = Mock(wraps=muse_args.add_muse_correction_args)
    monkeypatch.setattr(cli, "muse_option_defaults", supplier)
    monkeypatch.setattr(cli, "add_muse_backbone_args", backbone)
    monkeypatch.setattr(cli, "add_muse_correction_args", correction)
...
def test_explicit_disabled_flags_still_belong_to_muse_in_full_parser(
...
def test_checkpoint_parsing_keeps_provided_tracking_and_defers_cross_group_rules(
    ...
    with pytest.raises(SystemExit):
        _parse_training(monkeypatch, ["--speculator-type", "muse", *flags])
    ...
    assert restored._provided_model_config_dests & MUSE_OPTION_FIELDS == {
