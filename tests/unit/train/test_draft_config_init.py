...
def test_create_layer_config_layer_types(
    ...
    with patch(
        "speculators.train.draft_config.AutoConfig.from_pretrained",
...
def _patch_verifier(monkeypatch, hidden_size: int, vocab_size: int):
    monkeypatch.setattr(
        "speculators.train.draft_config.get_verifier_config",
...
def test_build_draft_model_mtp_from_scratch_uses_verifier_decoder(monkeypatch):
    ...
    verifier_cfg = object()
    monkeypatch.setattr(
        "speculators.train.model_init.get_verifier_config", lambda _p: verifier_cfg
    )
    ...
    monkeypatch.setattr(
        "speculators.train.model_init.create_transformer_layer_config", _must_not_call
    )
    monkeypatch.setattr(
        "speculators.train.model_init.resolve_mask_token_id", _must_not_call
    )
...
def _capture_full_attention_indices(
    ...
    monkeypatch.setattr(
        "speculators.train.model_init.create_transformer_layer_config", _fake_create
    )
    monkeypatch.setattr(
        "speculators.train.model_init.resolve_mask_token_id", lambda *_a, **_k: 0
    )
