"""Muse options agree across CLI, model construction and checkpoint overlays."""
...
def _config(config_class=MuseSpeculatorConfig, **options):
...
def _parse(monkeypatch, options=None, *, checkpoint=None, algorithm="muse"):
...
def _factory(monkeypatch, **options):
    ...
    return MuseDraftModel.from_training_args(
...
def test_defaults_match_cli_config_and_training_factory(monkeypatch):
    ...
    assert set(defaults) == MUSE_OPTION_FIELDS
    assert MUSE_OPTION_FIELDS.issubset(PRETRAINED_MODEL_CONFIG_FLAGS)
    ...
    assert muse_option_defaults()["enable_correction_head"] is False
...
def test_partial_checkpoint_options_still_validate_scalar_ranges(monkeypatch, options):
    with pytest.raises(ValueError):
        validate_muse_options(options, partial=True)
    with pytest.raises(SystemExit):
...
def test_checkpoint_actual_baseline_type_rejects_muse_override(
...
def test_mtp_weight_checkpoint_cannot_bypass_muse_option_guard(monkeypatch, tmp_path):
    template = _config()
    MTPSpeculatorConfig(
