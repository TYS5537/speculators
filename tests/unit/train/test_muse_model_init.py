...
def test_fresh_sources_route_decoder_then_mask_then_training_factory(
    ...
    args = _args(
        speculator_type="mtp" if source == "mtp" else "muse",
...
def test_config_only_uses_saved_class_then_mappings_and_verifier_weights(
    ...
    args = _args(
        from_pretrained="config-only", speculator_type="mtp" if mtp else "muse"
    )
...
def test_weight_restore_validates_before_selecting_loader(
    ...
    args = _args(from_pretrained="weights", speculator_type="mtp" if mtp else "muse")
