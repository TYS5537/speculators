...
# training arguments, independently transcribed (not extracted from settings.sh).
...
def test_baseline_resolves_only_the_four_requested_script_changes():
...
""")
    ...
    assert {
        key
        for key in baseline
        if baseline[key] != original[key] and key not in metadata
    } == {
        "lr",
        "weight_decay",
        "muon_weight_decay",
    ...
    assert baseline["muon_lr"] == 6e-4
    assert baseline["weight_decay"] == 0
    assert baseline["muon_weight_decay"] == 0
    ...
    assert baseline["optimizer"] == "muon"
    assert baseline["muon_parameter_policy"] == "legacy"
