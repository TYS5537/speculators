...
from speculators.train import vocab_setup as train_cli
...
def test_parse_vocab_mappings_broadcasts_to_nonzero_rank(
    ...
    def broadcast(payload, src):
        assert src == 0
        payload[0] = {"mappings": expected}

    broadcast_mock = Mock(side_effect=broadcast)
    monkeypatch.setattr(torch.distributed, "broadcast_object_list", broadcast_mock)
