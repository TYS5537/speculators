...
from speculators.models.muse import core as muse_core
...
class _ParallelHarness(nn.Module):
    _teacher_forced_parallel_correction = (
        MuseDraftModel._teacher_forced_parallel_correction
    )
...
def _tiny_model(sample_from_anchor, mode, with_selector):
    ...
    model = MuseDraftModel(config).train()
    with torch.no_grad():
...
def test_real_forward_keeps_anchor_and_projection_contract(
    ...
    original_metrics = muse_core.compute_metrics
    ...
    monkeypatch.setattr(muse_core, "compute_metrics", record_metrics)
