...
class _InputHarness:
    _validate_rollout_inputs = MMuseDraftModel._validate_rollout_inputs
    _initial_rollout_logit_feedback = MMuseDraftModel._initial_rollout_logit_feedback
    _rollout_token_embeddings = MMuseDraftModel._rollout_token_embeddings
    _rollout_correction_step = MMuseDraftModel._rollout_correction_step
    _rollout_correction_steps = MMuseDraftModel._rollout_correction_steps
    _draft_ids_to_verifier = MMuseDraftModel._draft_ids_to_verifier
    rollout_correction = MMuseDraftModel.rollout_correction
...
@pytest.mark.parametrize("hidden", [torch.zeros(2, 3, 4), torch.zeros(2, 4)])
def test_validation_requires_a_correction_head(hidden):
    model = _InputHarness()
    model.correction_head = None
    with pytest.raises(RuntimeError, match="enable_correction_head"):
        model._validate_rollout_inputs(**_validation_inputs(dflash_hidden=hidden))
...
@pytest.mark.parametrize(
    ...
    [
        (
            {
                "dflash_hidden": torch.zeros(2, 4),
                "anchor_token_ids": torch.zeros(2, 1),
                "conditioning_current_ids": torch.zeros(2, 2),
            },
            True,
            "dflash_hidden must be rank-3",
        ),
        (
            {
                "anchor_token_ids": torch.zeros(2, 1),
            ...
            True,
            "Expected anchor_token_ids shape (2,), got torch.Size([2, 1])",
        ),
        (
            {"conditioning_current_ids": torch.zeros(2, 2)},
            True,
            "Corrected Selector feedback cannot use a static path",
        ),
        (
            {"conditioning_current_ids": torch.zeros(2, 2)},
            False,
            "Expected conditioning_current_ids shape (2, 3), got (2, 2)",
        ),
        (
            {"conditioning_previous_ids": torch.zeros(2, 2)},
            False,
        ...
        (
            ...
            False,
            "Expected conditioning_previous_ids shape (2, 3), got (2, 2)",
        ),
        (
            {
                "conditioning_previous_rank_features": torch.zeros(2, 2, 2),
                "precomputed_base_logits": torch.zeros(2, 3, 5),
            },
            True,
        ...
        (
            {
                "conditioning_previous_rank_features": torch.zeros(2, 2, 2),
                "conditioning_previous_logits_mask": torch.zeros(2, 2),
                "precomputed_base_logits": torch.zeros(2, 3, 5),
            },
            True,
        ...
        (
            {
                "conditioning_previous_rank_features": torch.zeros(2, 2, 2),
                "conditioning_previous_logits_mask": torch.zeros(2, 3),
                "precomputed_base_logits": torch.zeros(2, 3, 5),
            },
            True,
            "Selector previous-rank features must align",
        ),
    ],
)
def test_validation_preserves_first_error_when_several_inputs_are_invalid(
    overrides, online, message
):
    model = _InputHarness(online=online)
...
@pytest.mark.parametrize("feedback", [None, "static", "corrected"])
@pytest.mark.parametrize("has_selector", [False, True])
def test_validation_online_mode_requires_a_selector_and_corrected_feedback(
    feedback, has_selector
):
    model = _InputHarness(online=has_selector)
    if feedback is None:
        del model.config.selector_correction_feedback
    else:
        model.config.selector_correction_feedback = feedback
    result = model._validate_rollout_inputs(
        **_validation_inputs(precomputed_base_logits=torch.zeros(2, 3, 6))
    )
    assert type(result) is tuple
    assert result == (has_selector and feedback == "corrected", False)
    assert model.correction_head.calls == []


@pytest.mark.parametrize("online", [False, True])
def test_validation_does_not_transform_noncontiguous_tensors_or_gradients(online):
    model = _InputHarness(online=online)
    source = torch.arange(42, dtype=torch.float64, requires_grad=True)
    features = (source * 2).view(3, 2, 7).transpose(0, 1)
    token_ids = torch.arange(6, dtype=torch.int32).view(3, 2).transpose(0, 1)
    arguments = _validation_inputs(
        dflash_hidden=torch.zeros(3, 2, 4, dtype=torch.float64).transpose(0, 1),
        conditioning_current_ids=None if online else token_ids,
        conditioning_previous_ids=None if online else token_ids,
        conditioning_previous_rank_features=features,
        # Mask dtype and compact width are intentionally not checked here.
        conditioning_previous_logits_mask=torch.ones(3, 2).transpose(0, 1),
        precomputed_base_logits=torch.zeros(3, 2, 6).transpose(0, 1),
    )
    tensors = {name: value for name, value in arguments.items() if value is not None}
    before = {name: value.clone() for name, value in tensors.items()}
    metadata = {
        name: (value.data_ptr(), value.dtype, value.device, value.stride())
        for name, value in tensors.items()
    }
    grad_fn = features.grad_fn
    assert model._validate_rollout_inputs(**arguments) == (online, True)
    for name, tensor in tensors.items():
        assert arguments[name] is tensor
    ...
    assert features.grad_fn is grad_fn
