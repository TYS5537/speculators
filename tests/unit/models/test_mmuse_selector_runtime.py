...
from speculators.models.mmuse import MMuseDraftModel
from speculators.models.mmuse.backbone import MMuseBackboneMixin
from speculators.models.mmuse.selector import DFlash2CandidateSelector
from speculators.models.mmuse.selector_runtime import MMuseSelectorMixin
...
class _SelectorHost(MMuseSelectorMixin, nn.Module):
    _draft_ids_to_verifier = MMuseBackboneMixin._draft_ids_to_verifier
...
def test_runtime_is_stateless_and_legacy_method_access_still_resolves():
    assert issubclass(MMuseBackboneMixin, MMuseSelectorMixin)
    assert not issubclass(MMuseSelectorMixin, nn.Module)
    assert "__init__" not in MMuseSelectorMixin.__dict__
    for name in (
        *_RUNTIME_METHODS,
        "_dflash2_select_greedy_path",
        "_dflash2_select_global_path",
    ):
        implementation = MMuseSelectorMixin.__dict__[name]
        assert name not in MMuseBackboneMixin.__dict__
        assert getattr(MMuseBackboneMixin, name) is implementation
        assert getattr(MMuseDraftModel, name) is implementation
        assert not hasattr(DSparkDraftModel, name)
        assert not hasattr(DFlashDraftModel, name)
    assert "_draft_ids_to_verifier" in MMuseBackboneMixin.__dict__
    assert "_draft_ids_to_verifier" not in MMuseSelectorMixin.__dict__
    assert MMuseDraftModel.__mro__.index(
        MMuseSelectorMixin
    ) < MMuseDraftModel.__mro__.index(DSparkDraftModel)
    host = _SelectorHost()
    direct = nn.Module()
