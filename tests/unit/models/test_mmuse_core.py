"""Focused unit tests for MMuse correction integration helpers."""
...
class _RolloutHarness:
    ...
    dflash2_select_candidates = MMuseDraftModel.dflash2_select_candidates
    _replace_compact_feature_position = staticmethod(
        MMuseDraftModel._replace_compact_feature_position
    )
    _selector_correction_inputs = MMuseDraftModel._selector_correction_inputs
...
class _CollaborationHarness:
    _apply_collaborative_markov = MMuseDraftModel._apply_collaborative_markov
...
def test_confidence_feature_detach_switch_controls_both_inputs():
    ...
    coupled = MMuseDraftModel._confidence_features(hidden, sequential, detach=False)
    coupled.sum().backward()
    assert hidden.grad is not None
    assert sequential.grad is not None

    detached = MMuseDraftModel._confidence_features(hidden, sequential, detach=True)
...
def test_hidden_alignment_loss_is_masked_and_zero_for_matching_states():
    ...
    loss = MMuseDraftModel._hidden_alignment_loss(corrected, verifier, mask)
