...
from speculators.models.mmuse.runtime_types import CorrectionRolloutOutput
...
class _ValidationHarness(nn.Module):
    _validation_correction_outputs = MMuseDraftModel._validation_correction_outputs
    ...
    def rollout_correction(self, hidden, **kwargs):
        ...
        return MMuseDraftModel.rollout_correction(self, hidden, **kwargs)
    ...
    def _rollout_correction_steps(self, hidden, anchor_token_ids, **kwargs):
        ...
        return CorrectionRolloutOutput(
            token_ids=self.rollout_blocks.argmax(dim=-1),
            logits=self.rollout_blocks,
            causal_states=hidden,
            corrected_hidden=hidden,
        )
...
class _LossHarness(nn.Module):
    _add_auxiliary_losses = MMuseDraftModel._add_auxiliary_losses
    ...
    def _hidden_alignment_loss(self, corrected, target, mask):
        loss = MMuseDraftModel._hidden_alignment_loss(corrected, target, mask)
