"""Architecture boundaries, disabled-feature parity and local MMuse round trips."""
...
def test_mmuse_is_registered_and_config_roundtrip_preserves_features():
    config = _config(
    ...
    assert type(restored) is MMuseSpeculatorConfig
    assert restored.to_dict() == config.to_dict()
    assert (
        SpeculatorModel.registered_model_class_from_config(restored) is MMuseDraftModel
    )
...
def test_baseline_training_factory_rejects_extensions_before_loading_verifier(
    ...
    with pytest.raises(ValueError, match="mmuse"):
...
def test_mmuse_training_factory_keeps_enabled_features(monkeypatch, feature):
    ...
    model = MMuseDraftModel.from_training_args(
        template.transformer_layer_config,
        verifier_name_or_path="unused-local-verifier",
        draft_vocab_size=32,
        target_layer_ids=[0, 1],
    ...
    assert model.config.speculators_config.algorithm == "mmuse"
...
def test_disabled_mmuse_preserves_baseline_initialization_and_backbone(
    baseline, sample_from_anchor
):
    if baseline == "dflash":
        config_class, model_class = DFlashSpeculatorConfig, DFlashDraftModel
        baseline_features = {}
        mmuse_features = {"markov_rank": 0, "enable_confidence_head": False}
    else:
        ...
        mmuse_features = baseline_features
    ...
    assert original.state_dict().keys() == mmuse.state_dict().keys()
    for name, value in original.state_dict().items():
        torch.testing.assert_close(
            value, mmuse.state_dict()[name], rtol=0, atol=0, equal_nan=True, msg=name
        )
    with torch.no_grad():
        for name in ("embed_tokens", "lm_head", "verifier_lm_head"):
            weight = getattr(original, name).weight
            weight.normal_(std=0.1)
            getattr(mmuse, name).weight.copy_(weight)
    inputs = {
    ...
    torch.manual_seed(123)
    actual = mmuse._backbone_forward(**inputs)
    ...
    if baseline != "dflash":
        ...
        torch.manual_seed(123)
        _, actual_loss, actual_metrics = mmuse(**inputs)
...
@pytest.mark.parametrize(
    "legacy_loader",
    [None, "generic", "dspark", "dspark_config", "muse", "muse_concrete"],
)
def test_mmuse_local_checkpoint_roundtrip_preserves_trainable_weights(
    ...
    if legacy_loader is not None:
        ...
        legacy_type = "muse" if legacy_loader.startswith("muse") else "dspark"
        legacy["speculators_model_type"] = legacy_type
        legacy["architectures"] = [
            "MuseSpeculator" if legacy_type == "muse" else "DSparkSpeculator"
        ]
        legacy["speculators_config"]["algorithm"] = legacy_type
        (tmp_path / "config.json").write_text(json.dumps(legacy), encoding="utf-8")
        if legacy_loader == "muse_concrete":
            loader = MMuseDraftModel
    ...
    warning_context = (
        pytest.warns(UserWarning, match="legacy (DSpark|Muse)")
        if legacy_loader is not None
        else nullcontext()
    )
    ...
    assert type(loaded) is MMuseDraftModel
    assert loaded.config.speculators_model_type == "mmuse"
    assert loaded.config.speculators_config.algorithm == "mmuse"
    ...
    resaved = json.loads((tmp_path / "resaved/config.json").read_text(encoding="utf-8"))
    assert resaved["speculators_model_type"] == "mmuse"
    assert resaved["speculators_config"]["algorithm"] == "mmuse"
    assert resaved["architectures"] == ["MMuseDraftModel"]


@pytest.mark.parametrize("enhanced", [False, True])
@pytest.mark.parametrize("architecture", ["MuseSpeculator", "MuseDraftModel"])
def test_legacy_muse_config_migrates_identity_only(enhanced, architecture):
    canonical = _config(MMuseSpeculatorConfig, enable_correction_head=enhanced)
    legacy = canonical.to_dict()
    legacy["speculators_model_type"] = "muse"
    legacy["architectures"] = [architecture]
    legacy["speculators_config"]["algorithm"] = "muse"
    original = copy.deepcopy(legacy)
    for loader in (SpeculatorModelConfig, MMuseSpeculatorConfig):
        with pytest.warns(UserWarning, match="legacy Muse"):
            restored = loader.from_dict(legacy)
        assert type(restored) is MMuseSpeculatorConfig
        assert restored.to_dict() == canonical.to_dict()
    assert legacy == original


def test_legacy_enhanced_dspark_config_migrates_without_changing_options():
    config = _config(MMuseSpeculatorConfig, enable_correction_head=True)
