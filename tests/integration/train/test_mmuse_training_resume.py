"""Real MMuse CPU training, transactional checkpoint restoration and continuation.

Only device placement and deterministic local verifier weights are adapted. The
registered MMuse model, Trainer loop, AdamW, LR scheduler and SingleGPUCheckpointer
...
def _make_model(config):
    ...
    model = MMuseDraftModel(config).to(dtype=torch.bfloat16)
...
def test_mmuse_train_save_resume_matches_uninterrupted(
    tmp_path, cpu_training, mode, legacy_identity
):
    _seed(71)
    ...
    assert type(first.model) is MMuseDraftModel
    ...
    assert saved_config["speculators_model_type"] == "mmuse"
    assert saved_config["speculators_config"]["algorithm"] == "mmuse"
    ...
    if legacy_identity:
        saved_config["speculators_model_type"] = "muse"
        saved_config["speculators_config"]["algorithm"] = "muse"
        saved_config["architectures"] = ["MuseDraftModel"]
        (checkpoint / "config.json").write_text(
            json.dumps(saved_config), encoding="utf-8"
        )
    _seed(999)
    with (
        pytest.warns(UserWarning, match="legacy Muse")
        if legacy_identity
        else nullcontext()
    ):
    ...
    assert type(restored_config) is MMuseSpeculatorConfig
    ...
    final_config = json.loads(
        (tmp_path / "resumed/1/config.json").read_text(encoding="utf-8")
    )
