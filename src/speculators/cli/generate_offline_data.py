...
def _validate_and_publish(
    source: str, target: Path, item: dict, validate: bool
) -> None:
    """Never expose partial or invalid output under a reusable cache filename."""
    loaded = None
    allow_prefix = "messages" in item
    if validate or allow_prefix:
        loaded = align_hidden_states(
            load_file(source), item["input_ids"], allow_prefix=allow_prefix
        )
    temporary = target.with_name(f".{target.name}.{uuid4().hex}.tmp")
    if loaded is None:
        shutil.move(source, temporary)
    else:
        save_file(loaded, temporary)
...
async def _generate_and_save_hidden_states(
    ...
    if validate_outputs:
        requested = set(
            get_indices_to_process(num_samples, max_samples, [], world_size, rank)
        )
        local_existing = [i for i in existing_file_indices if i in requested]
        valid_existing = await asyncio.to_thread(
            validate_existing_hidden_states, hidden_states_dir, dataset, local_existing
        )
