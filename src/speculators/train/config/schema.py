...
from speculators.models.mmuse.config import MMuseOptions
...
class DraftArgs(_Group):
    ...
    target_hidden_state_format: Literal["standard", "deepseek_v4_mean_hc_head"] = (
        "standard"
    )
...
class DataArgs(_Group):
    ...
    dsv4_external_arrow: bool = False
...
class LossArgs(_Group):
    loss_implementation: Literal["legacy", "fused", "eager"] | None = Field(
        default=None,
...
class TrainerArgs(_Group):
    activation_checkpointing: bool = Field(
        default=False, json_schema_extra=_CLI_BOOL_OPTIONAL
    )
...
class DSparkArgs(_Group):
    ...
    confidence_length_alpha: float = Field(default=0.0, ge=0.0)
    confidence_loss_weighting: Literal["uniform", "match-draft"] = "match-draft"
    first_error_focal_alpha: float = Field(default=0.0, ge=0.0)
    adaptive_loss: Literal["none", "cat", "ssal"] = "none"
    ssal_curriculum: bool = Field(default=False, json_schema_extra=_CLI_BOOL_OPTIONAL)
    ssal_curriculum_start: float = 0.1
    ssal_curriculum_end: float = 0.6
...
class MMuseArgs(MMuseOptions, _Group):
    """Architecture fields shared with saved MMuse configs, without duplication."""


for _field in MMuseArgs.model_fields.values():
    if _field.annotation is bool:
        _field.json_schema_extra = _CLI_BOOL_OPTIONAL
...
class TrainConfig(BaseSettings):
    ...
    training_recipe: Literal["legacy", "upstream"] | None = Field(
        default=None,
        description="Default policy: legacy for MMuse, upstream otherwise. "
        "Explicit values always win.",
    )
    mmuse: MMuseArgs = Field(default_factory=MMuseArgs)
    ...
    def _resolve_derived_defaults(self) -> "TrainConfig":
        ...
        """
        if self.speculator_type == "muse":
            self.speculator_type = "mmuse"
        if self.training_recipe is None:
            self.training_recipe = (
                "legacy" if self.speculator_type == "mmuse" else "upstream"
            )
        if self.training_recipe == "legacy":
            self._apply_legacy_defaults()
        if self.loss.loss_implementation is None:
            self.loss.loss_implementation = (
                "legacy" if self.training_recipe == "legacy" else "fused"
            )
        self._resolve_model_defaults()
        self._resolve_loss_defaults()
        return self

    def _apply_legacy_defaults(self) -> None:
        if "lr" not in self.optimizer.model_fields_set:
            self.optimizer.lr = 1e-4
        if self.optimizer.muon_lr is None:
            self.optimizer.muon_lr = 10 * self.optimizer.lr
        if "max_anchors" not in self.data.model_fields_set:
            self.data.max_anchors = 3072
        if self.dflash.per_position_loss_weight is None:
            self.dflash.per_position_loss_weight = "fixed-exp-decay"
        if self.speculator_type not in {"dspark", "mmuse"}:
            if self.dflash.block_size is None:
                self.dflash.block_size = 8
            if self.loss.loss_fn is None:
                self.loss.loss_fn = "kl_div"
            if self.draft.num_layers is None:
                self.draft.num_layers = 1
        if self.speculator_type in {"dspark", "mmuse"}:
            self._apply_legacy_dspark_defaults()

    def _apply_legacy_dspark_defaults(self) -> None:
        if self.dflash.block_size is None:
            self.dflash.block_size = 7
        if "dflash_decay_gamma" not in self.dflash.model_fields_set:
            self.dflash.dflash_decay_gamma = float(self.dflash.block_size)
        if "epochs" not in self.trainer.model_fields_set:
            self.trainer.epochs = 10
        if self.loss.loss_fn is None:
            self.loss.loss_fn = '{"ce": 0.1, "tv": 0.9}'
        if self.draft.num_layers is None:
            self.draft.num_layers = 5

    def _resolve_model_defaults(self) -> None:
    ...
    def _resolve_loss_defaults(self) -> None:
    ...
    def with_checkpoint_recipe(
        self, speculator_type: str, training_recipe: Literal["legacy", "upstream"]
    ) -> "TrainConfig":
        """Re-derive implicit defaults for a saved model without dropping overrides."""
        supplied = {
            key: value
            for key, value in self.flatten().items()
            if self.provenance.get(key, "default") != "default"
        }
        resolved = type(self).from_flat(
            {
                **supplied,
        ...
        resolved._provenance = {  # noqa: SLF001 -- clone this class's source metadata.
            **self.provenance,
            "speculator_type": "checkpoint",
            "training_recipe": "checkpoint",
        }
        resolved._argv = list(self.argv)  # noqa: SLF001
        resolved._backend_args = dict(self._backend_args)  # noqa: SLF001
        return resolved
    ...
    def from_flat(cls, flat: dict[str, Any]) -> "TrainConfig":
        ...
        known = {
            ...
            and not (
                value is None
                and dest in {"enable_confidence_head", "confidence_head_with_markov"}
            )
        }
        backend = {
