...
_ALGORITHM_GROUP_USERS: dict[str, frozenset[str]] = {
    "dflash": frozenset({"dflash", "dflash2", "dspark", "mmuse"}),
    "dflash2": frozenset({"dflash2"}),
    "dspark": frozenset({"dspark", "mmuse"}),
    "mmuse": frozenset({"mmuse"}),
...
def build_from_sources(
    ...
    *,
    ...
    _validate_extensions(cfg, provided)
    return cfg


def _validate_extensions(cfg: TrainConfig, provided: set[str]) -> None:
    if (
        cfg.speculator_type != "mmuse"
        and not cfg.draft.from_pretrained
        and provided & MMUSE_OPTION_FIELDS
    ):
        raise ConfigError("MMuse architecture options require --speculator-type mmuse")
    if cfg.speculator_type == "mmuse":
        try:
            validate_mmuse_options(
                cfg.flatten(), partial=bool(cfg.draft.from_pretrained)
            )
        except ValueError as exc:
            raise ConfigError(str(exc)) from exc
    if cfg.data.dsv4_external_arrow and (
        cfg.data.legacy_data
        or cfg.draft.target_hidden_state_format != "deepseek_v4_mean_hc_head"
    ):
        raise ConfigError("--dsv4-external-arrow requires DSV4 Arrow training")
