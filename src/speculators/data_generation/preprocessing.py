...
def _load_processor_for_rows(processor, dataset, target_model_path, trust_remote_code):
    """Encoded rows need no model/tokenizer files on the preprocessing machine."""
    if processor is not None or {"input_ids", "loss_mask"} <= set(dataset.column_names):
        return processor
    log.subsection("Loading processor")
    return load_processor(target_model_path, trust_remote_code=trust_remote_code)
...
def load_and_preprocess_dataset(
    ...
    *,
    ...
    for train_data_path in train_data_paths:
        ...
        processor = _load_processor_for_rows(
            processor, raw_dataset, target_model_path, trust_remote_code
        )
