...
class DFlash2DraftModel(DFlashDraftModel):
    ...
    def __init__(self, config: DFlash2SpeculatorConfig) -> None:
        ...
        if config.sample_from_anchor:
            raise ValueError(
                "Standalone DFlash2 requires sample_from_anchor=False so a "
                "block of 8 positions produces exactly 7 draft tokens."
            )
        ...
        self.candidate_selector = CandidateSelector(  # type: ignore[assignment]
    ...
    @property
    def _upstream_selector(self) -> CandidateSelector:
        """Return the standalone selector stored under its public weight prefix."""
        return cast(CandidateSelector, self.candidate_selector)
    ...
    def from_training_args(
        ...
        **kwargs,
        ...
        # Keep the standalone upstream model isolated from this branch's optional
        # DFlash/DSpark experiments. Its own decoder and selector are installed
        # below, and its serving contract uses bidirectional local SWA blocks.
        base_kwargs.update(
    ...
    def dflash2_select_candidates(
        ...
        """Expose the upstream selector through the shared offline-eval API."""
        return self._upstream_selector.select(
            logits,
            hidden_states,
            previous_token_ids,
        )
    ...
    def _prepare_missing_checkpoint_weights(self, loading_info: dict) -> None:
        """Fail closed when a purported DFlash2 checkpoint lacks trained modules."""
        ...
        missing_required = [
            key for key in missing if any(part in key for part in required_fragments)
        ]
        if missing_required:
            preview = ", ".join(missing_required[:8])
            suffix = " ..." if len(missing_required) > 8 else ""
            raise RuntimeError(
                "DFlash2 checkpoint is missing trained convolution/selector "
                f"weights: {preview}{suffix}"
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        selector = self._upstream_selector
