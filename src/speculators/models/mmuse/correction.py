"""MMUSE causal Correction head and its cached attention layers."""
...
from speculators.models.mmuse.correction_inputs import (
    validate_correction_embeddings,
    validate_hidden_feedback_inputs,
    validate_previous_logit_inputs,
)
...
class CausalCorrectionHead(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Register optional projections on this head, in checkpoint/RNG order.
        self._init_optional_projections(
            input_hidden_size=input_hidden_size,
            correction_hidden_size=correction_hidden_size,
            correction_rank=correction_rank,
            output_mode=output_mode,
            draft_vocab_size=draft_vocab_size,
            enable_hidden_auxiliary=enable_hidden_auxiliary,
            enable_hidden_feedback=enable_hidden_feedback,
        )
        self.residual_gate = nn.Linear(correction_hidden_size, 1)
        self._initialize_residual_parameters(gate_bias)
    ...
    def _init_optional_projections(
        self,
        *,
        input_hidden_size: int,
        correction_hidden_size: int,
        correction_rank: int,
        output_mode: Literal["hidden", "logits"],
        draft_vocab_size: int | None,
        enable_hidden_auxiliary: bool,
        enable_hidden_feedback: bool,
    ) -> None:
        """Register logit, feedback, then auxiliary-hidden projections.

        Keep names on the head itself and preserve construction order. Disabled
        projections remain ordinary None attributes, not registered modules.
        """
    ...
    def _initialize_residual_parameters(self, gate_bias: float) -> None:
        """Set residual/gate initial values after all modules are constructed."""
    ...
    def _lm_head_fusion_weights(
        ...
        if signature != self._lm_fusion_signature or self._lm_fusion_weight is None:
            lm_for_up = lm_head_weight.detach().to(
                device=up_weight.device,
                dtype=up_weight.dtype,
            )
            self._lm_fusion_signature = signature
            self._lm_fusion_weight = torch.matmul(lm_for_up, up_weight.detach())
    ...
    def _prepare_previous_rank_features(
        self,
        ...
        """Validate logit inputs, encode dense logits, then check rank width.

        Dense encoding stays before the rank-module/shape checks: moving those
        checks earlier would change error ordering and encoder-hook behavior.
        Compact features arrive already masked and must not be masked again.
        """
    ...
    def _build_correction_features(
        ...
        *,
        previous_rank_features: torch.Tensor | None,
        previous_corrected_hidden: torch.Tensor | None,
        previous_corrected_hidden_mask: torch.Tensor | None,
        current_token_embeddings: torch.Tensor | None,
    ) -> torch.Tensor:
        """Project validated inputs, retaining cast, addition and mask order."""
    ...
    def forward(
        ...
        *,
        ...
        validate_hidden_feedback_inputs(
            dflash_hidden,
            previous_corrected_hidden,
            previous_corrected_hidden_mask,
            enabled=self.hidden_feedback_proj is not None,
        )
        previous_rank_features = self._prepare_previous_rank_features(
            dflash_hidden,
            previous_logits,
            previous_logits_mask,
            previous_rank_features,
        )
        hidden_states = self._build_correction_features(
            previous_token_embeddings,
            dflash_hidden,
            block_positions,
