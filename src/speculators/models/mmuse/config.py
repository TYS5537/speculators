"""Configuration for the MMUSE extensions to DFlash and DSpark."""
...
from speculators.models.mmuse.backbone_config import MMuseBackboneConfigMixin


class MMuseOptions(MMuseBackboneConfigMixin):
...
MMUSE_OPTION_FIELDS = frozenset(MMuseOptions.model_fields)


def mmuse_option_defaults() -> dict[str, Any]:
    """Return independent option defaults without constructing a model config."""
    return {
        name: field.get_default(call_default_factory=True)
        for name, field in MMuseOptions.model_fields.items()
    }


def validate_mmuse_options(values: Mapping[str, Any], *, partial: bool = False) -> None:
    ...
    """
    options = MMuseOptions.model_validate(dict(values))
    if partial:
        return
...
def _validate_selector_dependencies(options: MMuseOptions, markov_rank: int) -> None:
    if (
...
def _validate_correction_dependencies(options: MMuseOptions) -> None:
    correction = options.enable_correction_head
...
@SpeculatorModelConfig.register("mmuse")
class MMuseSpeculatorConfig(MMuseOptions, DSparkSpeculatorConfig):
    """Optional backbone, Selector, Correction, and collaboration architecture.

    Existing flag and tensor names intentionally stay unchanged. Selecting MMUSE
    does not implicitly enable an experiment; use the explicit feature switches.
    """

    speculators_model_type: Literal["mmuse"] = "mmuse"  # type: ignore[assignment]
    architectures: list[str] = Field(default_factory=lambda: ["MMuseSpeculator"])
