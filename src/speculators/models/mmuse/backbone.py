"""MMuse backbone extensions composed with the baseline DSpark/DFlash model."""
...
from speculators.models.mmuse.decoder import Qwen3MMuseDecoderLayer
...
class MMuseBackboneMixin(MMuseSelectorMixin):
    """Add MMuse modules without making either baseline depend on MMuse."""

    _no_split_modules: ClassVar[list[str]] = [
        "Qwen3DFlashDecoderLayer",
        "Qwen3MMuseDecoderLayer",
    ]

    def _make_decoder_layer(self, config, layer_idx: int) -> Qwen3MMuseDecoderLayer:
        return Qwen3MMuseDecoderLayer(
    ...
    def _build_base_config_kwargs(
        ...
        if algorithm == "mmuse" and kwargs.get("sample_from_anchor") is None:
