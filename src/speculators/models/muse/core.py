...
@SpeculatorModel.register("muse")
class MuseDraftModel(MuseBackboneMixin, DSparkDraftModel):
    """MUSE: enhanced DFlash backbone with causal Correction and confidence.
    ...
    config_class: ClassVar[type[MuseSpeculatorConfig]] = MuseSpeculatorConfig  # type: ignore[misc,assignment]

    def _init_sequential_heads(self, config: MuseSpeculatorConfig) -> None:
        validate_muse_options(vars(config))
    ...
    def from_training_args(
        ...
        **kwargs,
        ...
        """Create the enhanced MUSE model without changing parameter names."""
        ...
        config = cls.config_class(
            **cls._build_base_config_kwargs("muse", verifier_config, **kwargs),
            **sequential_kwargs,
        )
    ...
    def _teacher_forced_parallel_correction(
        ...
        *,
        ...
        if correction_output_mode == "logits":
            ...
            if selector_previous_logits_mask is not None:
                ...
                if start_position == 0:
                    ...
                    previous_target_mask = torch.ones_like(
                        active_positions, dtype=torch.bool
                    )
        ...
        if delta_hidden is not None:
            # Keep the residual's slice separate from Correction's input slice:
            # sharing that autograd node changes gradient accumulation order.
            residual_hidden = (
                hidden_blocks
                if start_position == 0
                else hidden_blocks[:, start_position:]
            )
            ...
            if self.config.correction_project_corrected_hidden:
                ...
                if start_position:
                    residual = torch.cat(
                        [
                            residual.new_zeros(
                                num_blocks, start_position, residual.shape[-1]
                            ),
                            residual,
                        ],
                        dim=1,
                    )
                logits = projected_logits + residual.reshape(
            ...
            if position < start_position:
                ...
                if needs_previous_logits:
                    ...
                    if previous_rank_features is not None:
                        head_kwargs["previous_rank_features"] = previous_rank_features[
                            :, position : position + 1
                        ]
                    else:
    ...
    def _validate_rollout_inputs(
        ...
        *,
        ...
        """Validate the block inputs and identify online/compact conditioning."""
        if self.correction_head is None:
            raise RuntimeError(
    ...
    def _rollout_correction_steps(
        ...
        *,
        ...
        (
            ...
            online_previous_rank,
            online_previous_mask,
        ) = self._initial_rollout_logit_feedback(
            dflash_hidden,
            ...
            if self.config.correction_hidden_feedback:
                ...
                logits, correction_states, corrected_hidden = (
                    self._teacher_forced_parallel_correction(
                        ...
                        selector_current_embeddings=selector_current_embeddings,
